 ...
 *
 * This renders a compact row of buttons that lets the user swap between
 * multiple video views for the currently selected run.
 */

import type { SimulationViewOption } from '../data/simulations.ts';

export interface ViewSwitcherController {
  /** Replace the available view options and selected id. */
  update: (options: SimulationViewOption[], selectedId?: string) => void;
  /** Hide the switcher when the current simulation has no alternate views. */
  hide: () => void;
}
...
/**
 * Create a shared-styled SVG icon.
 *
 * @param content - Inner SVG markup.
 * @returns SVG element.
 */
